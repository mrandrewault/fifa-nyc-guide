 ...
 * MySpots.tsx
 * 
 * Shows the user's saved venues from localStorage.
 * Appears as a section in the Guide tab when the user has saved at least one venue.
 * 
 * Place this component in: src/components/shared/MySpots.tsx
 */
...
export default function MySpots() {
  ...
  const byBorough = favorites.reduce((acc, venue) => {
    if (!acc[venue.borough]) acc[venue.borough] = [];
    acc[venue.borough].push(venue);
    return acc;
  }, {} as Record<string, typeof favorites>);

  function openMaps(venue: typeof favorites[0]) {
    const url = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(venue.name + ' ' + venue.address)}`;
    window.open(url, '_blank');
  }

  function shareAll() {
    const text = `My World Cup NYC spots 🗽⚽\n\n${favorites.map(v => `• ${v.name} — ${v.address}`).join('\n')}\n\nvia golazo.nyc`;
    if (navigator.share) {
    ...
    <div className="mt-6 mb-2">
      ...
      <div className="flex items-center justify-between mb-3 px-1">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => {
              if (confirm('Clear all saved spots?')) clearFavorites();
            }}
            className="label text-[10px] px-3 py-1.5 rounded transition-colors"
            style={{
              background: '#1A1A1A',
      ...
      {Object.entries(byBorough).map(([borough, venues]) => (
        <div key={borough} className="mb-4">
          <div
            className="label text-[9px] mb-2 px-1"
            style={{ color: '#E8C84A', letterSpacing: '0.2em' }}
          >
            {borough.toUpperCase()}
          </div>
